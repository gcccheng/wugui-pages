...
Red Hat 认证架构师（RHCA）｜Red Hat 2024 年度认证专业人士 (RHCP of the Year)｜平台工程师 ｜ 平台架构/治理 ｜ AIOps/Platform Ops ｜ Developer Experience 工程师  
...
<a href="https://www.credly.com/users/gang-cheng.7632b4b0">Credly</a>
</div>

## 概要（Summary）

程刚是一位拥有十多年实战经验的基础设施与平台工程师，长期工作于本地数据中心、虚拟化环境、云原生基础设施以及完整的 <span style="color:red;"><strong>Red Hat 技术生态</strong></span>。他具备 <span style="color:red;"><strong>RHEL</strong></span>、<span style="color:red;"><strong>Red Hat Satellite</strong></span>、<span style="color:red;"><strong>OpenShift / OCP</strong></span>、<span style="color:red;"><strong>OpenShift Virtualization</strong></span> 相关培训、<span style="color:red;"><strong>Ansible Automation Platform</strong></span>、Kubernetes、<span style="color:red;"><strong>裸金属基础设施</strong></span>、<span style="color:red;"><strong>Linux 与 Windows 系统管理</strong></span>、<span style="color:red;"><strong>IaC</strong></span>、<span style="color:red;"><strong>CI/CD 流水线</strong></span>、可观测性与安全加固等方面的实践能力，尤其擅长在复杂环境中推进平台建设、运维治理、故障排查与基础设施迁移工作。

他的经历包括支撑运行在 <span style="color:red;"><strong>VMware vSphere / ESX</strong></span> 上的大规模 Red Hat 虚拟机环境、构建和运营 <span style="color:red;"><strong>裸金属</strong></span> Kubernetes 平台、验证并加固 VMware 模板镜像、自动化操作系统升级与补丁管理，以及与基础设施团队、开发团队和业务方协作，推动低风险、可审计的变更落地。他也具备 <span style="color:red;"><strong>虚拟机迁移规划</strong></span>、<span style="color:red;"><strong>容量评估</strong></span>、以及 <span style="color:red;"><strong>操作系统、网络、存储层面的故障排查</strong></span> 经验。通过在 Sopra Steria 为包括 <span style="color:red;"><strong>投资银行</strong></span> 在内的客户提供平台相关工作，他也积累了受监管金融环境中的实践，理解在 <span style="color:red;"><strong>合规</strong></span>、<span style="color:red;"><strong>安全</strong></span> 与 <span style="color:red;"><strong>稳定性</strong></span> 要求都很高的场景下，为什么 Red Hat 技术栈更容易满足可控、可审计、可维护的要求。

2024 年，他被授予 <span style="color:red;"><strong>Red Hat Certified Professional of the Year 2024</strong></span>，这是 Red Hat 面向全球认证工程师颁发的年度最高技术荣誉，当年全球仅此一人。这一奖项体现了他在 Red Hat Linux、OpenShift、Ansible 自动化与平台工程方面的技术深度。结合多项 Red Hat 认证、课程与 OpenShift Virtualization 相关培训经历，程刚具备较强的故障分析能力、结构化沟通能力与务实执行风格，能够在复杂工程组织中稳定交付结果。
...
#### ★ 自主平台 SRE 智能体（Autonomous Platform SRE Agent - AI 运营 MVP）

设计并构建了一个模块化的自主 AI 智能体，旨在减少“琐事（Toil）”并自动化重复性的平台工程任务，实现从“被动告警”到“主动修复”的模式转变。

**职责（Responsibilities）**

- 架构了基于 **Python 的协调器（Orchestrator）模式**，支持可插拔的“专家模块”（Kubernetes、Proxmox、供应链），用于处理多域运营任务。
- 实现了 **“大脑与工具”架构**，智能体主动扫描基础设施 API（K8s、Proxmox）并识别违规行为，利用 LLM (GPT-4o) 进行故障推理并生成修复方案，而非仅仅报告错误。
- 构建了 **安全引擎与策略看门人（Safety Engine & Policy Gatekeeper）**，确保 AI 的自主权受限：智能体可自动修复安全项（如重启停滞的虚拟机），但对高风险变更（如破坏性补丁）则需要人工审批。
- 开发了基于 **Flask/HTMX 的“任务控制”仪表盘（Mission Control Dashboard）**，提供 AI “思维链”的透明度，使工程师能够审查并一键执行建议的修复方案。
- 解决了复杂的“不可变 Pod”修补挑战，通过 **语义推理** 教会智能体识别并修补父级控制器（Deployments/StatefulSets），而非瞬时生成的 Pod。
- 集成了 **供应链智能模块**，可自动扫描 Terraform/Ansible 代码，读取 GitHub Release Notes，并在建议版本升级前进行语义风险分析。

**价值（Value Created）**

- **减少运营琐事**：自动化了数百个安全策略违规（Kyverno）和基础设施健康检查的分类与修复工作。
- **维护工作左移**：将枯燥的依赖更新任务转化为“审查与批准”流，显著加速了平台版本的迭代。
- **证明 AI 投资回报**：证明了 AI 可以安全执行“L1 级” SRE 任务，从而将资深工程师的时间解放出来用于更核心的架构设计工作。
...
- 设计并构建基于 **Rancher + Flatcar 的高可用裸金属 Kubernetes 平台**，提供标准化的平台底座与能力边界。  
- 牵头平台能力规划与演进路线图，推动跨团队对齐平台边界与服务目录。  
- 使用 **GitLab CI、Argo CD、Kustomize** 实现 GitOps 工作流，将交付流程平台化与可审计化。  
- 通过 **Terraform** 标准化基础设施资源的创建与管理，并配合 CI/CD 流水线实现自动化交付与变更治理。  
- 平台化运营关键能力组件：Harbor 镜像仓库、TrueNAS NFS 存储、ExternalDNS、Bind9、MetalLB、Yocto 相关构建环境以及 GitLab Runners 等。  
